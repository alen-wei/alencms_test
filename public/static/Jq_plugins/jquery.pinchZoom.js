...
function showImgZoom(picurl){
	var html='<div class="zoompicdiv_obj">';
	html+='<div class="imgbox"><i class="icon-spinner icon-spin"></i></div>';
	html+='</div>';
	var obj=$(html);
	$('body').append(obj);
	loadImg(picurl,function(img){
		obj.find('.imgbox').html(img);
		obj.find('.imgbox').height(obj.find('.imgbox').height());
		obj.find('.imgbox').click(function(){
			obj.remove();
		});
		obj.find('.imgbox').each(function () {
