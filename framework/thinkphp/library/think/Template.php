...
class Template
{
            ...
            while ($matches[0]) {
                ...
                if (isset($_varParseList[$match[0]])) {
                    ...
                    if (strpos($match[0], '.')) {
                        ...
                        /* Alen 支持$Alen标签等同于$Think */
						//if ('$Think' == $first) {
                        if ('$Think' == $first or '$Alen' == $first) {
						/* Alen end */
    ...
    private function parseTemplateFile($template)
    {
       /*
		 * alen
		 * 文件名以#开头自动定位到当前主题的当前语言文件夹
		 * 文件名以&开头自动定位到public文件夹
		*/
		$alenZf=(0 === strpos($template,'#') or 0 === strpos($template,'&'))?substr($template,0,1):false;
		$suffix='' == pathinfo($template, PATHINFO_EXTENSION)?false:true;
		if(!$suffix or $alenZf){
			...
			if(!$suffix)$template.='.' . ltrim($this->config['view_suffix'], '.');
        }
		/* alen End */
        if (is_file($template)) {
            // 记录模板文件的更新时间
            ...
            switch ($tagName) {
                ...
                case 'extend':
                   ...
                   /* alen include标签支持以#或&开头 */
					...
					$lessReg=($tagName=='include' or $tagName=='layout')?'\#\&':'';
					*/
					$lessReg='\#\&';
                    if ($single) {
                        $regex = $begin . $tagName . '\b(?>(?:(?!' . $name . '=).)*)\b' . $name . '=([\'\"])(?P<name>['.$lessReg.'\$\w\-\/\.\:@,\\\\]+)\\1(?>[^' . $end . ']*)' . $end;
                    } else {
                        $regex = $begin . $tagName . '\b(?>(?:(?!' . $name . '=).)*)\b' . $name . '=([\'\"])(?P<name>['.$lessReg.'\$\w\-\/\.\:@,\\\\]+)\\1(?>(?:(?!' . $end . ').)*)' . $end;
                    }
					/* alen End */
